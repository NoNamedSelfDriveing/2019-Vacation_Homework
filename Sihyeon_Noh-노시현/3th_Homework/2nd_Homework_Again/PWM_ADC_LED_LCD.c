...
int main(void)
{
	...
	while (1)
	{
		...
		ledPower = (float)ADC_READ() * 255.0 / 1023; //255단위로 값 변환
		OCR0 = ledPower;							//출력 비교 레지스터에 값 대입
...
void IO_SETTING()
{
	...
	DDRB |= (1<<PORTB4);	//PORTB4를 출력으로 설정 PWM LED
}
